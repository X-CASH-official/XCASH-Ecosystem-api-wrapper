...
def get_amount(atomic: int) -> float:
...
def get_atomic(xcash_amount) -> int:
...
def process_destinations(destinations: list) -> list:
    ...
    for d in destinations:
        ...
        if all(key in formated_destination for key in allowed):
            if not isinstance(formated_destination.get("amount"), (int, float)):
                ...
                formated_destination["amount"] = get_atomic(formated_destination.get("amount"))
